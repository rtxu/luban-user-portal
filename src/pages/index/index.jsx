...
const AppOperation = ({text, record, index, onDeleteApp, onChangeDescription}) => {
  const handleDelete = () => {
    Modal.confirm({
...
const AppList = ({ apps, onDeleteApp, onChangeDescription }) => {
  const columns = [
    { title: '名称', dataIndex: 'name', key: 'name', className: styles.appColumn },
...
const Index = ({ apps, onAddApp, onDeleteApp, initApps, onChangeDescription }) => {
  ...
  return (
    <div className={styles.container}>
      <h2>应用列表</h2>
      <AppList apps={apps} onDeleteApp={onDeleteApp} onChangeDescription={onChangeDescription} />
      <div className={styles.appListOperationZone}>
        <Button type='primary' size='large' onClick={showModal}>新建</Button>
        <CreateAppForm
