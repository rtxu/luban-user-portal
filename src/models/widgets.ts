...
function _deleteOne(widgets, widgetIdToDelete) {
  const leftWidgetsId = Object.keys(widgets).filter(
    id => id !== widgetIdToDelete
  );
  const newWidgets = {};
...
export default {
    ...
    *loadWidgets(action, sagaEffects) {
    ...
    saveWidgets: [
      function*(action, sagaEffects) {
        ...
        logger.debug(
          `in ${NS}/saveWidgets effect, targetAction: `,
          targetAction
        );
        const widgets = yield select(state => state.widgets);

        const resp = yield call(
          widgetsService.saveWidgets,
          userId,
          appId,
          widgets
        );
  ...
  reducers: {
    ...
    [updateWidgetContentType]: (widgets, action) => {
      ...
      logger.debug(
        `updateWidgetContent(widgetId=${widgetId}, widgetAction=${widgetAction})`
      );
      if (widgetId in widgets) {
        ...
        return {
          ...
          [widgetId]: {
            ...widget,
            content: WidgetFactory.getReducer(widget.type)(
              widget.content,
              widgetAction
            )
          }
    ...
    [changeWidgetIdType]: (widgets, action) => {
      ...
      logger.debug(
        `changeWidgetId(oldWidgetId=${oldWidgetId}, newWidgetId=${newWidgetId})`
      );
