...
function OneLineOverflowText({ text }) {
  ...
  return (
    <div className={styles.widgetOneLineOverflowText}>
      <div ref={textDivRef} className={isOverflow? styles.overflown : styles.mightOverflow} >{text}</div>
      {
        isOverflow && (
...
OneLineOverflowText.defaultProps = {
  text: '这里有一个超级长长长长长长长长长长长长长长长长长长长长长长长长长长长长长长长长长长长长长长长长长长长长长长长长长长长长长长长长长长长长的文本，用以测量是否会 overflow',
};
...
function reducer(prevState, action) {
  switch (action.type) {
    ...
    default:
      throw new Error(`in OneLineOverflowTextWidget reducer(): unexpected action type: ${action.type}`);
  }
}
...
OneLineOverflowText.use = () => {
  const [widgetProps, widgetDispatch] = useReducer(OneLineOverflowText.reducer, OneLineOverflowText.initialState);
  return ([<OneLineOverflowText {...widgetProps} />, 
    widgetProps, 
  <OneLineOverflowText.ConfigPanel dispatch={widgetDispatch} {...widgetProps} />]);
}
