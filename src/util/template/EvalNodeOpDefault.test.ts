...
test('listPossibleDepId', () => {
  const node = new EvalNode('widget1.value', EvalNodeTypeEnum.Default, ' {{a.b.c}} {{aa.bb.cc}} {{aaa.b.ccc}}');
...
test('error happened when evaluate', () => {
  const testEvaluateError = (input, expectedErrorType) => {
    ...
    const node = new EvalNode('widget1.value', EvalNodeTypeEnum.Default, input, mockOnEval);
    Op.evaluate(node, {});
...
test('evaluate template_string', () => {
  ...
  const depNode = new EvalNode('textinput1.value', EvalNodeTypeEnum.Default, '');
